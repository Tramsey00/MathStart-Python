"""Publish the revised rational-number lesson beside its original edition."""
import sqlite3
...
from content.lessons.v2.rational import PAGE
...
def navigation(body):
    ...
    original = soup.select_one('#deistvitelnye-chisla .ms-topic[href="/10-klass-racionalnye-chisla-povtorenie/"]')
    if original is None:
        raise CommandError('В программе не найдена тема 1.2 о рациональных числах.')
    link = soup.select_one(f'.ms-topic[href="/{PAGE["slug"]}/"]')
    if link is None:
        link = BeautifulSoup(f'<a class="ms-topic" href="/{PAGE["slug"]}/"><span class="ms-topic-number">1.2</span><span class="ms-topic-name">{PAGE["title"]} 2.0</span></a>', 'html.parser').a
    original.insert_after(link.extract())
    return course_html(str(soup))


class Command(BaseCommand):
    help = 'Добавляет рациональные числа 2.0 и ссылку в разделе 1, сохраняя прежний урок.'
    ...
    def handle(self, *args, **options):
        ...
        original = ContentPage.objects.get(slug='10-klass-racionalnye-chisla-povtorenie')
        ...
        backup = Path(settings.BASE_DIR) / 'data' / 'backup_before_rational_v2.sqlite3'
        ...
        self.stdout.write(self.style.SUCCESS('Добавлена тема «Рациональные числа. Повторение 2.0». Прежний урок сохранён.'))
